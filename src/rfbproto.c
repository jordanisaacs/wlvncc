...
static rfbBool HandleUltraZip16(rfbClient* client, int rx, int ry, int rw,
                                int rh);
static rfbBool HandleUltraZip32(rfbClient* client, int rx, int ry, int rw,
                                int rh);
...
static rfbBool HandleTRLE24Down(rfbClient* client, int rx, int ry, int rw,
                                int rh);
...
static rfbBool HandleZRLE24Down(rfbClient* client, int rx, int ry, int rw,
                                int rh);
...
{
	return (client->supportedMessages.client2server[((messageType & 0xFF) / 8)] &
	                        (1 << (messageType % 8))
	                ? TRUE
	                : FALSE);
}
...
{
	return (client->supportedMessages.server2client[((messageType & 0xFF) / 8)] &
	                        (1 << (messageType % 8))
	                ? TRUE
	                : FALSE);
}
...
{
	client->supportedMessages.client2server[((messageType & 0xFF) / 8)] |=
	        (1 << (messageType % 8));
}
...
{
	memset((char*)&client->supportedMessages, 0,
	       sizeof(client->supportedMessages));
...
rfbBool ConnectToRFBRepeater(rfbClient* client, const char* repeaterHost,
                             int repeaterPort, const char* destHost, int destPort)
{
	...
	rfbClientLog(
	        "Connected to VNC repeater, using protocol version %d.%d\n",
	        major, minor);

	memset(tmphost, 0, sizeof(tmphost));
	if (snprintf(tmphost, sizeof(tmphost), "%s:%d", destHost, destPort) >=
	    (int)sizeof(tmphost))
...
static void ReadReason(rfbClient* client)
{
	...
	if (reasonLen > 1 << 20) {
		rfbClientLog("VNC connection failed, but sent reason length of "
		             "%u exceeds limit of 1MB",
		             (unsigned int)reasonLen);
		return;
	}
	reason = malloc(reasonLen + 1);
	if (!reason || !ReadFromRFBServer(client, reason, reasonLen)) {
		free(reason);
		return;
	}
	reason[reasonLen] = 0;
...
static rfbBool ReadSupportedSecurityType(rfbClient* client, uint32_t* result,
                                         rfbBool subAuth)
{
	uint8_t count = 0;
	uint8_t loop = 0;
	...
	if (!ReadFromRFBServer(client, (char*)&count, 1))
		return FALSE;

	if (count == 0) {
		rfbClientLog("List of security types is ZERO, expecting an "
		             "error to follow\n");
		ReadReason(client);
		return FALSE;
	}
	...
	for (loop = 0; loop < count; loop++) {
		if (!ReadFromRFBServer(client, (char*)&tAuth[loop], 1))
			return FALSE;
		...
		switch (tAuth[loop]) {
		case rfbUltra:
			rfbClientLog("UltraVNC server detected, enabling "
			             "UltraVNC specific messages\n");
			DefaultSupportedMessagesUltraVNC(client);
			break;
		case rfbTight:
			rfbClientLog("TightVNC server detected, enabling "
			             "TightVNC specific messages\n");
		...
		for (e = rfbClientExtensions; e; e = e->next) {
			...
			for (secType = e->securityTypes; secType && *secType;
			     secType++) {
		...
		if (tAuth[loop] == rfbVncAuth || tAuth[loop] == rfbNoAuth ||
		    ...
		    (!subAuth &&
		     (tAuth[loop] == rfbTLS || tAuth[loop] == rfbVeNCrypt)) ||
#endif
	...
	if (authScheme == 0) {
		...
		for (loop = 0; loop < count; loop++) {
			if (strlen(buf1) >= sizeof(buf1) - 1)
				break;
			snprintf(buf2, sizeof(buf2), (loop > 0 ? ", %d" : "%d"),
			         (int)tAuth[loop]);
...
{
	if (cred->userCredential.username)
		free(cred->userCredential.username);
	if (cred->userCredential.password)
...
static rfbBool HandlePlainAuth(rfbClient* client)
{
	...
	ulen = (cred->userCredential.username
	                ? strlen(cred->userCredential.username)
	                : 0);
	ulensw = rfbClientSwap32IfLE(ulen);
	plen = (cred->userCredential.password
	                ? strlen(cred->userCredential.password)
	                : 0);
	...
	if (ulen > 0) {
		if (!WriteToRFBServer(client, cred->userCredential.username,
		                      ulen)) {
			FreeUserCredential(cred);
			return FALSE;
		}
	...
	if (plen > 0) {
		if (!WriteToRFBServer(client, cred->userCredential.password,
		                      plen)) {
			FreeUserCredential(cred);
			return FALSE;
		}
...
static rfbBool HandleUltraMSLogonIIAuth(rfbClient* client)
{
	uint8_t gen[8], mod[8], resp[8], pub[8], priv[8];
	uint8_t username[256], password[64], key[8];
	rfbCredential* cred;

	if (!ReadFromRFBServer(client, (char*)gen, sizeof(gen)))
		return FALSE;
	if (!ReadFromRFBServer(client, (char*)mod, sizeof(mod)))
		return FALSE;
	if (!ReadFromRFBServer(client, (char*)resp, sizeof(resp)))
		return FALSE;

	if (!dh_generate_keypair(priv, pub, gen, sizeof(gen), mod, sizeof(priv))) {
		rfbClientErr("HandleUltraMSLogonIIAuth: generating keypair "
		             "failed\n");
		return FALSE;
	}

	if (!dh_compute_shared_key(key, priv, resp, mod, sizeof(key))) {
		rfbClientErr("HandleUltraMSLogonIIAuth: creating shared key "
		             "failed\n");
		return FALSE;
	}
	...
	rfbClientLog("WARNING! MSLogon security type has very low password "
	             "encryption! "
	...
	strncpy((char*)username, cred->userCredential.username,
	        sizeof(username) - 1);
	memset(password, 0, sizeof(password));
	strncpy((char*)password, cred->userCredential.password,
	        sizeof(password) - 1);
	FreeUserCredential(cred);
	...
	if (!WriteToRFBServer(client, (char*)pub, sizeof(pub)))
		return FALSE;
	if (!WriteToRFBServer(client, (char*)username, sizeof(username)))
		return FALSE;
	if (!WriteToRFBServer(client, (char*)password, sizeof(password)))
		return FALSE;
...
static rfbBool HandleMSLogonAuth(rfbClient* client)
{
	...
	if (!ReadFromRFBServer(client, (char*)&gen, 8))
		return FALSE;
	if (!ReadFromRFBServer(client, (char*)&mod, 8))
		return FALSE;
	if (!ReadFromRFBServer(client, (char*)&resp, 8))
		return FALSE;
	...
	if (!WriteToRFBServer(client, (char*)&pub, 8))
		return FALSE;
	if (!WriteToRFBServer(client, (char*)username, sizeof(username)))
		return FALSE;
	if (!WriteToRFBServer(client, (char*)password, sizeof(password)))
...
static rfbBool HandleARDAuth(rfbClient* client)
{
	uint8_t gen[2], len[2];
	size_t keylen;
	uint8_t *mod = NULL, *resp = NULL, *priv = NULL, *pub = NULL,
	        *key = NULL, *shared = NULL;
	...
	mod = (uint8_t*)malloc(
	        keylen *
	        5); /* the block actually contains mod, resp, pub, priv and key */
	if (!mod)
		goto out;
	...
	if (!ReadFromRFBServer(client, (char*)resp, keylen)) {
		rfbClientErr("HandleARDAuth: reading peer's generated public "
		             "key failed\n");
		goto out;
	}
	...
	/* Step 3: Perform Diffie-Hellman key agreement, using the generator
	   (gen), prime (mod), and the peer's public key. The output will be a
	   shared secret known to both us and the peer. */
	...
	if (!client->GetCredential) {
		rfbClientErr(
		        "HandleARDAuth: GetCredential callback is not set\n");
		goto out;
	}
	cred = client->GetCredential(client, rfbCredentialTypeUser);
	if (!cred) {
...
void SetClientAuthSchemes(rfbClient* client, const uint32_t* authSchemes, int size)
{
	...
	if (authSchemes) {
		if (size < 0) {
			/* If size<0 we assume the passed-in list is also
			 * 0-terminate, so we calculate the size here */
			for (size = 0; authSchemes[size]; size++)
				;
		}
		client->clientAuthSchemes =
		        (uint32_t*)malloc(sizeof(uint32_t) * (size + 1));
		if (client->clientAuthSchemes) {
			for (i = 0; i < size; i++)
...
rfbBool InitialiseRFBConnection(rfbClient* client)
{
	...
	if (major == 3 && (minor == 4 || minor == 6)) {
		rfbClientLog("UltraVNC server detected, enabling UltraVNC "
		             "specific messages\n",
		             pv);
		DefaultSupportedMessagesUltraVNC(client);
	}
	...
	if (major == 3 && (minor == 14 || minor == 16)) {
		minor = minor - 10;
		client->minor = minor;
		rfbClientLog("UltraVNC Single Click server detected, enabling "
		             "UltraVNC specific messages\n",
		             pv);
		DefaultSupportedMessagesUltraVNC(client);
	}
	...
	/* left in for backwards compatibility */
	if (major == 3 && minor == 5) {
		rfbClientLog("TightVNC server detected, enabling TightVNC "
		             "specific messages\n",
		             pv);
		DefaultSupportedMessagesTightVNC(client);
	}
	...
	case rfbTLS:
		if (!HandleAnonTLSAuth(client))
			return FALSE;
		/* After the TLS session is established, sub auth types are
		 * expected. Note that all following reading/writing are through
		 * the TLS session from here.
		 */
		...
		default:
			rfbClientLog("Unknown sub authentication scheme from "
			             "VNC server: %d\n",
			             (int)subAuthScheme);
			return FALSE;
		}
		...
		for (e = rfbClientExtensions; e; e = e->next) {
			...
			for (secType = e->securityTypes; secType && *secType;
				...
				if (authScheme == *secType) {
					if (!e->handleAuthentication(client,
					                             authScheme))
						return FALSE;
					if (!rfbHandleAuthResult(client))
						return FALSE;
	...
	if (!WriteToRFBServer(client, (char*)&ci, sz_rfbClientInitMsg))
		return FALSE;

	if (!ReadFromRFBServer(client, (char*)&client->si, sz_rfbServerInitMsg))
		return FALSE;

	client->si.framebufferWidth =
	        rfbClientSwap16IfLE(client->si.framebufferWidth);
	client->si.framebufferHeight =
	        rfbClientSwap16IfLE(client->si.framebufferHeight);
	client->si.format.redMax = rfbClientSwap16IfLE(client->si.format.redMax);
	client->si.format.greenMax =
	        rfbClientSwap16IfLE(client->si.format.greenMax);
	client->si.format.blueMax =
	        rfbClientSwap16IfLE(client->si.format.blueMax);
	client->si.nameLength = rfbClientSwap32IfLE(client->si.nameLength);

	if (client->si.nameLength > 1 << 20) {
		rfbClientErr("Too big desktop name length sent by server: %u B "
		             "> 1 MB\n",
	...
	if (!ReadFromRFBServer(client, client->desktopName, client->si.nameLength))
		return FALSE;
...
rfbBool SetFormatAndEncodings(rfbClient* client)
{
	...
	do {
		...
		if (strncasecmp(encStr, "raw", encStrLen) == 0) {
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingRaw);
		} else if (strncasecmp(encStr, "copyrect", encStrLen) == 0) {
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingCopyRect);
#ifdef LIBVNCSERVER_HAVE_LIBZ
#ifdef LIBVNCSERVER_HAVE_LIBJPEG
		} else if (strncasecmp(encStr, "tight", encStrLen) == 0) {
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingTight);
			requestLastRectEncoding = TRUE;
			if (client->appData.compressLevel >= 0 &&
			...
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingHextile);
#ifdef LIBVNCSERVER_HAVE_LIBZ
		} else if (strncasecmp(encStr, "zlib", encStrLen) == 0) {
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingZlib);
			if (client->appData.compressLevel >= 0 &&
			    client->appData.compressLevel <= 9)
				requestCompressLevel = TRUE;
		} else if (strncasecmp(encStr, "zlibhex", encStrLen) == 0) {
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingZlibHex);
			if (client->appData.compressLevel >= 0 &&
			    client->appData.compressLevel <= 9)
				requestCompressLevel = TRUE;
		} else if (strncasecmp(encStr, "trle", encStrLen) == 0) {
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingTRLE);
		} else if (strncasecmp(encStr, "zrle", encStrLen) == 0) {
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingZRLE);
		} else if (strncasecmp(encStr, "zywrle", encStrLen) == 0) {
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingZYWRLE);
			requestQualityLevel = TRUE;
#endif
			...
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingUltra);
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingUltraZip);
		} else if (strncasecmp(encStr, "corre", encStrLen) == 0) {
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingCoRRE);
	...
	if (se->nEncodings < MAX_ENCODINGS && requestCompressLevel) {
		encs[se->nEncodings++] =
		        rfbClientSwap32IfLE(client->appData.compressLevel +
		                            rfbEncodingCompressLevel0);
	}

	if (se->nEncodings < MAX_ENCODINGS && requestQualityLevel) {
		if (client->appData.qualityLevel < 0 ||
		    client->appData.qualityLevel > 9)
			client->appData.qualityLevel = 5;
		encs[se->nEncodings++] = rfbClientSwap32IfLE(
		        client->appData.qualityLevel + rfbEncodingQualityLevel0);
	}
	...
	if (client->appData.useRemoteCursor) {
		if (se->nEncodings < MAX_ENCODINGS)
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingXCursor);
		if (se->nEncodings < MAX_ENCODINGS)
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingRichCursor);
		if (se->nEncodings < MAX_ENCODINGS)
			encs[se->nEncodings++] =
			        rfbClientSwap32IfLE(rfbEncodingPointerPos);
	}
	...
	if (se->nEncodings < MAX_ENCODINGS)
		encs[se->nEncodings++] =
		        rfbClientSwap32IfLE(rfbEncodingSupportedMessages);
	if (se->nEncodings < MAX_ENCODINGS)
		encs[se->nEncodings++] =
		        rfbClientSwap32IfLE(rfbEncodingSupportedEncodings);
	if (se->nEncodings < MAX_ENCODINGS)
		encs[se->nEncodings++] =
		        rfbClientSwap32IfLE(rfbEncodingServerIdentity);
	...
	if (se->nEncodings < MAX_ENCODINGS)
		encs[se->nEncodings++] =
		        rfbClientSwap32IfLE(rfbEncodingQemuExtendedKeyEvent);
...
rfbBool SendIncrementalFramebufferUpdateRequest(rfbClient* client)
{
	return SendFramebufferUpdateRequest(
	        client, client->updateRect.x, client->updateRect.y,
...
rfbBool SendFramebufferUpdateRequest(rfbClient* client, int x, int y, int w,
                                     int h, rfbBool incremental)
{
	rfbFramebufferUpdateRequestMsg fur;

	if (!SupportsClient2Server(client, rfbFramebufferUpdateRequest))
		return TRUE;
	...
	if (!WriteToRFBServer(client, (char*)&fur,
...
rfbBool TextChatSend(rfbClient* client, char* text)
{
	...
	if (!SupportsClient2Server(client, rfbTextChat))
		return TRUE;
...
rfbBool TextChatOpen(rfbClient* client)
{
	...
	return (WriteToRFBServer(client, (char*)&chat, sz_rfbTextChatMsg)
	                ? TRUE
	                : FALSE);
}
 ...
 * Apparently, the remote client can *prevent* the local user from interacting
 * with the display I would think this is extremely helpful when used in a
 * HelpDesk situation
 */
rfbBool PermitServerInput(rfbClient* client, int enabled)
{
	...
	if (!SupportsClient2Server(client, rfbSetServerInput))
		return TRUE;
...
rfbBool SendXvpMsg(rfbClient* client, uint8_t version, uint8_t code)
{
	...
	if (!SupportsClient2Server(client, rfbXvp))
		return TRUE;
	xvp.type = rfbXvp;
	xvp.pad = 0;
	xvp.version = version;
...
rfbBool SendPointerEvent(rfbClient* client, int x, int y, int buttonMask)
{
	...
	if (!SupportsClient2Server(client, rfbPointerEvent))
		return TRUE;
...
static rfbBool ResizeClientBuffer(rfbClient* client, int width, int height)
{
	client->width = width;
...
rfbBool SendExtDesktopSize(rfbClient* client, uint16_t width, uint16_t height)
{
	...
	if (client->screen.width == 0 && client->screen.height == 0) {
		rfbClientLog("Screen not yet received from server - not "
		             "sending dimensions %dx%d\n",
		             width, height);
		return TRUE;
	}

	if (client->screen.width != rfbClientSwap16IfLE(width) ||
		...
		if (!WriteToRFBServer(client, (char*)&sdm,
		                      sz_rfbSetDesktopSizeMsg))
			return FALSE;
		if (!WriteToRFBServer(client, (char*)&screen,
...
rfbBool SendKeyEvent(rfbClient* client, uint32_t key, rfbBool down)
{
	...
	if (!SupportsClient2Server(client, rfbKeyEvent))
		return TRUE;
...
rfbBool SendExtendedKeyEvent(rfbClient* client, uint32_t keysym,
                             uint32_t keycode, rfbBool down)
{
	rfbQemuExtendedKeyEventMsg ke;

	/* FIXME: rfbQemuEvent also covers audio events, but this model for
	 * checking for supported messages is somewhat limited, so I'll leave
	 * this as is for now.
	 */
	if (!SupportsClient2Server(client, rfbQemuEvent))
		return FALSE;
	...
	return WriteToRFBServer(client, (char*)&ke,
...
rfbBool SendClientCutText(rfbClient* client, char* str, int len)
{
	...
	if (!SupportsClient2Server(client, rfbClientCutText))
		return TRUE;
...
rfbBool HandleRFBServerMessage(rfbClient* client)
{
	...
	case rfbServerCutText: {
		...
		if (msg.sct.length > 1 << 20) {
			rfbClientErr("Ignoring too big cut text length sent by "
			             "server: %u B > 1 MB\n",
		...
		if (!buffer ||
		    !ReadFromRFBServer(client, buffer, msg.sct.length)) {
			free(buffer);
			return FALSE;
		}
	...
	case rfbTextChat: {
		...
		case rfbTextChatOpen:
			...
			if (client->HandleTextChat != NULL)
				client->HandleTextChat(
				        client, (int)rfbTextChatOpen, NULL);
			break;
		case rfbTextChatClose:
			rfbClientLog("Received TextChat Close\n");
			if (client->HandleTextChat != NULL)
				client->HandleTextChat(
				        client, (int)rfbTextChatClose, NULL);
			break;
		case rfbTextChatFinished:
			rfbClientLog("Received TextChat Finished\n");
			if (client->HandleTextChat != NULL)
				client->HandleTextChat(
				        client, (int)rfbTextChatFinished, NULL);
			break;
		default:
			...
			if (!buffer ||
			    !ReadFromRFBServer(client, buffer, msg.tc.length)) {
				free(buffer);
				return FALSE;
			}
			...
			if (client->HandleTextChat != NULL)
				client->HandleTextChat(
				        client, (int)msg.tc.length, buffer);
			free(buffer);
			break;
		}
		break;
	...
	case rfbXvp: {
		...
		if (client->HandleXvpMsg)
			client->HandleXvpMsg(client, msg.xvp.version,
			                     msg.xvp.code);
...
#define GET_PIXEL16(pix, ptr)                                                  \
	(((uint8_t*)&(pix))[0] = *(ptr)++, ((uint8_t*)&(pix))[1] = *(ptr)++)

#define GET_PIXEL32(pix, ptr)                                                  \
	(((uint8_t*)&(pix))[0] = *(ptr)++, ((uint8_t*)&(pix))[1] = *(ptr)++,   \
	 ((uint8_t*)&(pix))[2] = *(ptr)++, ((uint8_t*)&(pix))[3] = *(ptr)++)
...
void PrintPixelFormat(rfbPixelFormat* format)
{
	if (format->bitsPerPixel == 1) {
		...
		rfbClientLog("  %s significant bit in each byte is leftmost on "
		             "the screen.\n",
		...
		if (format->trueColour) {
			rfbClientLog(
			        ...
			        format->redMax, format->greenMax,
			        format->blueMax, format->redShift,
			        format->greenShift, format->blueShift);
		} else {
